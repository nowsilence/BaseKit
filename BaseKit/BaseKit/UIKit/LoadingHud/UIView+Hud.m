...
@property(nonatomic) NCKHudState hudeState;

@property(nonatomic) int padding;

@property(nonatomic,strong) NSDictionary *infoDict;

@property(nonatomic,copy) void (^block)(NCKHudState state);

- (void)adjustFrame;

- (void)setInfoText:(NSString *)text hudState:(NCKHudState)state;
...
- (void)nck_setHudState:(NCKHudState)state
{
    [self nck_setHudState:state hudImage:nil];
}

- (void)nck_setHudState:(NCKHudState)state hudImage:(UIImage *)image
{
    NCKHudView *hud = [self associateHudView];
    
    if (state == NCKHudStateRemove) {
...
- (void)nck_setOnTapBlock:(void (^)(NCKHudState state))block
{
    NCKHudView *hud = [self associateHudView];
...
- (void)nck_setInfoText:(NSString *)text hudState:(NCKHudState)state
{
    NCKHudView *hud = [self associateHudView];
...
{
    if (self.block
        && (self.hudeState == NCKHudStateNothing || self.hudeState == NCKHudStateErr)) {
        
        self.block(self.hudeState);
    }
}

- (void)setHudeState:(NCKHudState)hudeState
{
...
- (void)adjustFrame
{
    ...
    if (self.hudeState == NCKHudStateLoading) {
...
- (NSDictionary *)infoDict
{
    ...
    _infoDict = @{@(NCKHudStateLoading):@"正在加载...",
                  @(NCKHudStateNothing):@"加载的数据为空！",
                  @(NCKHudStateErr):@"加载失败，点击重新载入！"};
